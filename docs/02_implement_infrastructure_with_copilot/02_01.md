...
1. From Chat with Copilot enter the following prompt to create an issue:
    ...
    deploy to the App Service, so I'll need a Container Registry as well. But I don't want to install docker on my local machine. The appService should use
    Azure RBAC to pull images not passwords. I want to monitor it with
    Application Insights. I will be using GPT-4 and Phi so I will need Microsoft Foundry, I want to use swedencentral because they are available there. Everything should be deployed together in one resource group in the same region. This is a dev environment and I want to use AZD with Bicep to define and deploy everything.
    Add SecurityControl and CostControl tags set to "Ignore". SecurityControl : Ignore  CostControl : Ignore
    ```
