...
1. From Chat with Copilot enter the following prompt to create an issue:
    ...
    Foundry, I want to use eastus because they are available there. Everything
...
application (dev environment) into a single resource group in eastus. Use
...
eastus as these models are available there.

## Requirements
- All resources provisioned into a single resource group (e.g. `rg-zavastore-dev-eastus`) in eastus
...
- Microsoft Foundry resource provisioned in eastus
...
## Checklist
- [ ] Confirm region and subscription quotas for Microsoft Foundry in eastus
