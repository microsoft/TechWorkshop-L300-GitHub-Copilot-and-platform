...
1. **Verify All Variables Exist**
   ...
   > **NOTE** Your Copilot-generated workflow may use different variable names. Ensure the variable names you configure in GitHub match exactly what your workflow YAML references.
...
1. **Verify Dockerfile Location**
   - Ensure Dockerfile exists at the **repository root** (not in `src/`)
   - Check the build context in the workflow (typically `.` for the repo root)
